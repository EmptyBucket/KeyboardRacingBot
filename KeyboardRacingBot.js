...
function randomTime(seed, dispersionPercent) {
	var dispersion = seed * dispersionPercent / 100;
	while(true)
    	yield Math.random() * dispersion * 2 + seed - dispersion;
}

function randomMinMax(max, min) {
	while(true)
		yield (Math.random() * (max - min) | 0) + min;
}

function textWriter() {
	game.addEventListener("racing", function() {
		var fakeError = "♀";

		var text = ["typefocus", "afterfocus"]
			.map(item => document.getElementById(item))
		        .map(item => item.children.length && Array.prototype.slice.call(item.children) || item)
			.reduce((prev, cur) => prev.concat(cur), [])
			.filter(item => item.style.display !== "none")
			.reduce((prev, cur) => prev + cur.textContent, "");

		var countFakeError = randomMinMax(10, 0).next();
		console.log(countFakeError);
		for(var i = 0; i < countFakeError; i++) {
			var position = randomMinMax(text.length, 0).next();
			var fake = randomMinMax(1040, 1103).next();
			text = [ text.slice(0, position), String.fromCharCode(fake) + fakeError, text.slice(position) ].join("");
		}
		console.log(text);

		var keyMap = { a: "а", x: "х", e: "е", c: "с", y: "у", k: "к", o: "о", p: "р", A: "А", X: "Х", E: "Е", C: "С", T: "Т", B: "В", H: "Н", K: "К", O: "О", P: "Р" };

		var rand = randomTime(100, 40);

		var delayText = (text) => {
		  var iter = 0;
		  (function delayChar(char) {
		    setTimeout(function() { 
		      
			var ruChar = keyMap[char] || char;
			inputtext.value = ruChar === fakeError ? inputtext.value.slice(0, inputtext.value.length - 1) : inputtext.value + ruChar;
...
document.addEventListener("beforescriptexecute", function(event) {
	...
	if(new RegExp(/js\/play.js/).test(script.src)) {
		console.log("injection script play.js");

		var textFunction = executeFakeScript.toString();
		var bodyFunction = textFunction.slice(textFunction.indexOf("{") + 1, textFunction.lastIndexOf("}"));
		var newScript = document.createElement("script");
		newScript.type = "text/javascript";
		newScript.textContent = bodyFunction;

		var parent = script.parentElement;
		parent.replaceChild(newScript, script);

		setTimeout(textWriter, 0);

		event.preventDefault();
	}
});

function executeFakeScript() {
	console.log("injection script play.js success");

	var game = {text: 'You\'re so curious... there is nothing interesting. But we will block your account and IP. Just in case :)'};
	var params = {};

	(function(scopeup) {
	  scopeup.init = function(id, params) {
	    delete this.init;

	    game = new Game(id);

	    for(var key in params) {
	      //if(~['human', 'voc_myrating', 'speedpanel_scale'].indexOf(key))
	        game[key] = params[key];
	    }
	    ...
	    [
	      ...
	      scopeup[func] = function() {
	        return game[func].apply(game, arguments);
	      };
	    });
	...
	<td>\
	<div class="car" app:player-car="#{user.carData}"></div>\
	</td>\
	</tr>\
	</table>\
	<div class=divider><img src="/img/blank.gif"></div>\
	...
	({
		...
		update: function(info)
		{
			...
			if(oldinfo)
			{
				...
				{
	                $$$('#car'+info.id+' .imgcont').addClass('leave');
				}
				if(oldinfo.leave && !info.leave)
				{
	                $$$('#car'+info.id+' .imgcont').removeClass('leave');
				}
			}
	...
	({
		...
		loadInfo: function(json)
		{
			...
			if(this.custom)
			{
				...
				$$$('.gameblock .gametype-sign').addClass('sign-'+this.params.gametype_clean+' active');
			}
			else
				this.params = {};
		...
		changeStatus: function(status)
		{
	        ...
	        if(status == 'racing')
			{
				...
				this.events["racing"].forEach(item => item());
			}
				...
				for(var i=0;i<this.places.length;i++)
				{
										...
										if($('newrecord_ub6'))
										{
											var newrecord_img_update = setInterval(function()
											{
												if(game.userpanel_updated)
												{
													...
													$('newrecord_ub2').src = 'http://i.klavogonki.ru/userbar/ub2-'+info.user.id+'.gif?'+info.speed;
													$('newrecord_ub6').src = 'http://i.klavogonki.ru/userbar/ub6-'+info.user.id+'.gif?'+info.speed;
												}
											}, 3000);
										}
					...
					else
					{
						...
						if(game.params && game.params.competition && info.user && game.ratings && game.ratings[info.user.id])
						{
							...
							var elm_rating_gained = $('rating_gained'+info.id);
	                        elm_rating_gained.update(tplRatingGained.evaluate(info));
	                        
	                        $$$(elm_rating_gained).find('.rating_gained').attr('title', 'Полученный опыт').tipsy({gravity: 'w'});
							...
							if(player.you)
							{
								try {
	                                ...
	                                var title = $('userpanel-level').getAttribute('original-title');
	                                var level = title.match(/Уровень: ([\d]+)/);
	                                if(level)
	                                ...
	                                if(level) {
	                                    $('userpanel-level').setAttribute( 'title', sprintf('Уровень: %s<br />Опыт: %s', level, game.ratings[info.user.id].pd));
	                                }
	                                $('userpanel-level-value').update(game.ratings[info.user.id].l);
	                                $('userpanel-level-value-shadow').update(game.ratings[info.user.id].l);
	                            } catch(e) {
	                                // bug of the 80 level (?)
	                                // see http://klavogonki.ru/forum/problems/1511/
	                            }
							}
						}
					}
	  ...
	  getGametype: function() {
	    return this.custom ? this.params.gametype : 'normal';
	  },
	...
	document.addEventListener('DOMContentLoaded', function() {
	  ...
	  if(inputtext) {
	    inputtext.addEventListener('keyup', change);
	  }
	});
	...
	function change(event)
	{
		...
		if(!game.skiptrackerror)
		{
			...
			{
				if(!game.error && lastLength < el.value.length)
				{
					game.error = true;
					game.errors++;
					$('errors-label').update(game.errors);
					if(params.sound)
						new Audio('/typo.mp3').play();
